...
List<VersionParameters> getAllVersions() {
  ...
  const int owaspRecommendedPbkdf2Sha512IterationsVersion1 =
      120000; // OWASP recommendation: https://cheatsheetseries.owasp.org/cheatsheets/Password_Storage_Cheat_Sheet.html#pbkdf2
  ...
  versions.add(VersionParameters(
      ...
      (passphrase) => _pbkdf2(
          ...
          Hmac.sha512(),
          owaspRecommendedPbkdf2Sha512IterationsVersion1,
      ...
      (adk, salt) => _pbkdf2(
          ...
          Xchacha20.poly1305Aead().secretKeyLength),
      (key, nonce, plaintext) =>
          _xChaCha20Poly1305Encrypt(key, nonce, plaintext, aad),
  ...
  version = 2;
  aad = protocol + version.toString();
  const int owaspRecommendedPbkdf2Sha512IterationsVersion2 =
      210000; // OWASP recommendation: https://cheatsheetseries.owasp.org/cheatsheets/Password_Storage_Cheat_Sheet.html#pbkdf2
  versions.add(VersionParameters(
      version,
      (passphrase) => _pbkdf2(
          passphrase,
          Uint8List.fromList(utf8.encode(aad)),
          Hmac.sha512(),
          owaspRecommendedPbkdf2Sha512IterationsVersion2,
          Xchacha20.poly1305Aead().secretKeyLength),
      (adk, salt) => _pbkdf2(
          adk,
          salt,
...
VersionParameters getVersion(int? version) {
  ...
  final VersionParameters foundVersion = versions.firstWhere(
      (item) => item.version == version,
      orElse: () => throw BadProtocolVersion());
  return foundVersion;
}
...
class PackageData {
  ...
  PackageData(this.parameters, this.pbkdfSalt, this.aeadNonce, this.aeadTag,
      this.ciphertext);
...
class VersionParameters {
  ...
  Future<SecretBox> Function(Uint8List, Uint8List, Uint8List)
...
Future<PackageData> _parseBytes(Uint8List bytes) async {
  ...
  final PackageData data =
      PackageData(parameters, pbkdfSalt, aeadNonce, aeadTag, ciphertext);
...
/// PBKDF2
Future<Uint8List> _pbkdf2(Uint8List passphrase, Uint8List salt,
    MacAlgorithm macAlgorithm, int iterations, int derivedKeyLength) async {
  ...
  final SecretKey derivedKey =
      await pbkdf.deriveKey(secretKey: SecretKey(passphrase), nonce: salt);
...
Future<SecretBox> _xChaCha20Poly1305Encrypt(
    Uint8List key, Uint8List nonce, Uint8List plaintext, String aad) async {
  final Xchacha20 aead = Xchacha20.poly1305Aead();
  final SecretBox ciphertext = await aead.encrypt(plaintext,
...
Future<Uint8List> _xChaCha20Poly1305Decrypt(
    Uint8List key, SecretBox blob, String aad) async {
  final Xchacha20 aead = Xchacha20.poly1305Aead();
  try {
    final List<int> plaintext = await aead.decrypt(blob,
...
Future<Tuple2<int, Uint8List>> generateAdk(String passphrase,
    {int? version}) async {
  ...
  Uint8List adk =
      await parameters.adkPbkdf(Uint8List.fromList(utf8.encode(passphrase)));
...
Future<Tuple2<PackageData, Uint8List>> encryptRawWithPassphrase(
    String passphrase, Uint8List plaintext,
    {int? version}) async {
  // Get version parameters; this can fail
  final VersionParameters parameters = getVersion(version);
...
Future<Tuple2<PackageData, Uint8List>> encryptRawWithAdk(
    Uint8List adk, Uint8List plaintext,
    {int? version}) async {
  // Get version parameters; this can fail
  final VersionParameters parameters = getVersion(version);
  ...
  final Uint8List pbkdfSalt = Uint8List.fromList(List<int>.generate(
      parameters.pbkdfSaltSize, (_) => rng.nextInt(0xFF + 1)));
  final Uint8List aeadKey = await parameters.pbkdf(adk, pbkdfSalt);

  // Use the AEAD key to encrypt the plaintext
  final Uint8List aeadNonce = Uint8List.fromList(List<int>.generate(
      parameters.aeadNonceSize, (_) => rng.nextInt(0xFF + 1)));
  final SecretBox blob =
      await parameters.aeadEncrypt(aeadKey, aeadNonce, plaintext);

  // Assemble data and add the checksum
  final PackageData data = PackageData(parameters, pbkdfSalt, aeadNonce,
      Uint8List.fromList(blob.mac.bytes), Uint8List.fromList(blob.cipherText));
...
Future<Uint8List> encryptWithPassphrase(String passphrase, Uint8List plaintext,
    {int? version}) async {
  final Tuple2<PackageData, Uint8List> raw =
      await encryptRawWithPassphrase(passphrase, plaintext, version: version);
...
Future<Uint8List> encryptWithAdk(Uint8List adk, Uint8List plaintext,
    {int? version}) async {
  final Tuple2<PackageData, Uint8List> raw =
      await encryptRawWithAdk(adk, plaintext, version: version);
  final PackageData data = raw.item1;
  final Uint8List checksum = raw.item2;
...
Future<Uint8List> decryptWithPassphrase(
    String passphrase, Uint8List bytes) async {
  // Parse the bytes into data; this can fail
  PackageData data = await _parseBytes(bytes);

  // Use the PBKDF to derive an ADK from the passphrase
  final Uint8List adk = await data.parameters
      .adkPbkdf(Uint8List.fromList(utf8.encode(passphrase)));
...
Future<Uint8List> decryptWithAdk(Uint8List adk, Uint8List bytes) async {
  ...
  final blob =
      SecretBox(data.ciphertext, nonce: data.aeadNonce, mac: Mac(data.aeadTag));
