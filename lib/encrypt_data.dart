...
Future<Uint8List> encrypt(SimplePublicKey recipient, Uint8List data) async {
  ...
  final SimplePublicKey ephemeralPublicKey =
      await ephemeralKey.extractPublicKey();

  // Produce an ECDH shared secret against the recipient public key
  final SecretKey sharedSecret = await X25519()
      .sharedSecretKey(keyPair: ephemeralKey, remotePublicKey: recipient);
  ...
  final List<int> nonce = List<int>.generate(
      Xchacha20.poly1305Aead().nonceLength, (_) => rng.nextInt(0xFF + 1));
  final SecretBox ciphertext = await Xchacha20.poly1305Aead().encrypt(data,
      secretKey: SecretKey(aeadKey),
      nonce: nonce,
      aad: utf8.encode("Stack Wallet data encryption AEAD ") +
...
Future<Uint8List> decrypt(SimpleKeyPair key, Uint8List data) async {
  ...
  // We need enough data
  if (data.length <
      KeyPairType.x25519.publicKeyLength +
  ...
  final Uint8List ephemeralPublicKey =
      data.sublist(counter, counter + KeyPairType.x25519.publicKeyLength);
  counter += KeyPairType.x25519.publicKeyLength;
  final Uint8List nonce =
      data.sublist(counter, counter + Xchacha20.poly1305Aead().nonceLength);
  ...
  final SecretKey sharedSecret = await X25519().sharedSecretKey(
      keyPair: key,
      remotePublicKey:
          SimplePublicKey(ephemeralPublicKey, type: KeyPairType.x25519));

  // Use a KDF to produce an AEAD key from the shared secret
  final List<int> aeadKey = await kdf(sharedSecret);

  // Decrypt the data with an AEAD
  final SecretBox aeadData = SecretBox(ciphertext, nonce: nonce, mac: Mac(mac));
  try {
    final List<int> plaintext = await Xchacha20.poly1305Aead().decrypt(aeadData,
        secretKey: SecretKey(aeadKey),
        aad: utf8.encode("Stack Wallet data encryption AEAD ") +
            ephemeralPublicKey +
            publicKey.bytes);
