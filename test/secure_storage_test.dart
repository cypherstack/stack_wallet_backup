...
void main() {
  ...
  test('upgrade, version 1 to 2', () async {
    ...
    StorageCryptoHandler handler =
        await StorageCryptoHandler.fromNewPassphrase(passphrase, 1);
    ...
    final String decryptedValue =
        await handler.decryptValue(name, encryptedValue);
  ...
  for (int version in [1, 2]) {
    ...
    test('examples, version $version', () async {
      ...
      const String passphrase = 'test';
      StorageCryptoHandler handler =
          await StorageCryptoHandler.fromNewPassphrase(passphrase, version);
      ...
      String encryptedValue =
          await handler.encryptValue(name, value, padding: padding);
      ...
      expect(() => handler.decryptValue(name, evilEncryptedValue),
          throwsA(const TypeMatcher<BadDecryption>()));
      ...
      handler =
          await StorageCryptoHandler.fromExisting(passphrase, keyBlob, version);
      ...
      expect(
          () => StorageCryptoHandler.fromExisting(
              incorrectPassphrase, keyBlob, version),
          throwsA(const TypeMatcher<IncorrectPassphraseOrVersion>()));
      ...
      handler = await StorageCryptoHandler.fromExisting(
          newPassphrase, newKeyBlob, version);
    });
    ...
    test('padding, version $version', () async {
      ...
      for (int i = 1; i <= 2 * padding + 1; i++) {
        ...
        int paddedBytesLength = base64.decode(encryptedValue).length -
            1 // padding flag
            -
            4 // encoded length
            -
    ...
    test('failures, version $version', () async {
      ...
      expect(
          () => StorageCryptoHandler.fromExisting(
              evilPassphrase, keyBlob, version),
          throwsA(const TypeMatcher<IncorrectPassphraseOrVersion>()));

      // Bad key blob size
      final String badKeyBlob = base64.encode(_randomBytes(saltLength +
          Xchacha20.poly1305Aead().nonceLength +
          Poly1305().macLength)); // too short
      expect(
          () => StorageCryptoHandler.fromExisting(
              passphrase, badKeyBlob, version),
          throwsA(const TypeMatcher<InvalidLength>()));

      // Evil key blob
      final String evilKeyBlob = base64.encode(_randomBytes(saltLength +
          Xchacha20.poly1305Aead().nonceLength +
          Xchacha20.poly1305Aead().secretKeyLength +
          Poly1305().macLength));
      expect(
          () => StorageCryptoHandler.fromExisting(
              passphrase, evilKeyBlob, version),
          throwsA(const TypeMatcher<IncorrectPassphraseOrVersion>()));
      ...
      for (int evilVersion in [1, 2]) {
        ...
        expect(
            () => StorageCryptoHandler.fromExisting(
                passphrase, evilKeyBlob, evilVersion),
            throwsA(const TypeMatcher<IncorrectPassphraseOrVersion>()));
      }
      ...
      expect(() => handler.decryptValue(evilName, encryptedValue),
          throwsA(const TypeMatcher<BadDecryption>()));

      // Bad encrypted value size
      String badEncryptedValue =
          base64.encode(_randomBytes(Xchacha20.poly1305Aead().nonceLength +
              1 // padding flag
              +
              ...
              +
              Poly1305().macLength));
      expect(() => handler.decryptValue(name, badEncryptedValue),
          throwsA(const TypeMatcher<BadDecryption>()));

      // Evil encrypted value
      String evilEncryptedValue =
          base64.encode(_randomBytes(Xchacha20.poly1305Aead().nonceLength +
              1 // padding flag
              +
              utf8.encode(value).length +
